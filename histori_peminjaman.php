...
<table class="table table-hover table-striped">
	...
	<tbody>
		...
		while ($dt_histori=mysqli_fetch_array($qry_histori)) {
			...
			$ps_dipinjam="<ol>";
			$qry_ps=mysqli_query($conn,"select * from detail_peminjaman_ps join ps on ps.id_ps=detail_peminjaman_ps.id_ps where id_peminjaman_ps = '".$dt_histori['id_peminjaman_ps']."'");
		...
		if (mysqli_num_rows($qry_cek_kembali)>0) {
			...
			$button_kembali="<a href='pengembalian.php?id=".$dt_histori['id_peminjaman_ps']."' class='btn btn-warning' onclick='return confirm(\"Yakin ingin kembalikan?\")'>Kembalikan</a>";
		}
		?>
		<tr>
